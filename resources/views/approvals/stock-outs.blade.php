            ...
            <div>
                <nav class="flex text-sm text-gray-500 mb-2" aria-label="Breadcrumb">
                    <ol class="flex items-center space-x-2">
                        ...
                        <li><i class="ti ti-chevron-right text-xs"></i></li>
                        <li class="text-gray-800 font-medium">Stock Out Approvals</li>
                    </ol>
                </nav>
                <h2 class="text-2xl font-bold text-gray-900">Stock Out Approvals</h2>
                <p class="text-sm text-gray-500 mt-1">Review and approve pending stock out transactions</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-orange-600 mt-2" id="total-quantity">0</p>
                    </div>
                    <div class="bg-orange-100 rounded-full p-3">
                        <i class="ti ti-arrow-up text-orange-600 text-2xl"></i>
                    </div>
                </div>
            </div>
        </div>
                ...
                <table id="approvals-table" class="w-full">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">Total Quantity</th>
    ...
    <script>
        $(document).ready(function() {
            const table = $('#approvals-table').DataTable({
                ...
                bPaginate: false,
                ajax: {
                    url: "{{ route('approvals.stock-outs') }}",
                    data: function(d) {
                        d.status = 'PENDING';
            ...
            function updateSummary() {
                $.get("{{ route('approvals.stock-outs') }}", { status: 'PENDING', summary: true }, function(data) {
                    if (data.summary) {
                        $('#pending-count').text(data.summary.pending || 0);
                        $('#approved-count').text(data.summary.approved_today || 0);
                        $('#total-quantity').text(parseFloat(data.summary.total_quantity || 0).toLocaleString('id-ID', {minimumFractionDigits: 2}));
                    }
                });
            }
        });
