            ...
            <div>
                <nav class="flex text-sm text-gray-500 mb-2" aria-label="Breadcrumb">
                    <ol class="flex items-center space-x-2">
                        ...
                        <li><i class="ti ti-chevron-right text-xs"></i></li>
                        <li class="text-gray-800 font-medium">Stock Adjustment Approvals</li>
                    </ol>
                </nav>
                <h2 class="text-2xl font-bold text-gray-900">Stock Adjustment Approvals</h2>
                <p class="text-sm text-gray-500 mt-1">Review and approve pending stock adjustment transactions</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-500">Positive Adjustments</p>
                        <p class="text-3xl font-bold text-emerald-600 mt-2" id="positive-count">0</p>
                    </div>
                    <div class="bg-emerald-100 rounded-full p-3">
                        <i class="ti ti-arrow-up text-emerald-600 text-2xl"></i>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-500">Negative Adjustments</p>
                        <p class="text-3xl font-bold text-red-600 mt-2" id="negative-count">0</p>
                    </div>
                    <div class="bg-red-100 rounded-full p-3">
                        <i class="ti ti-arrow-down text-red-600 text-2xl"></i>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="p-6 border-b border-gray-200">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="w-full sm:w-64">
                        <input type="search" id="approvalsSearch" placeholder="Search adjustments..."
                ...
                <table id="approvals-table" class="w-full">
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-right text-xs font-semibold text-gray-600 uppercase tracking-wider">Value Impact</th>
    ...
    <script>
        $(document).ready(function() {
            const table = $('#approvals-table').DataTable({
                ...
                bPaginate: false,
                ajax: {
                    url: "{{ route('approvals.stock-adjustments') }}",
                    data: function(d) {
                        d.status = 'DRAFT';
                ...
                columns: [
                    ...
                    { 
                        ...
                        render: function(data) {
                            ...
                            return `<span class="${color} font-semibold">${sign}Rp ${Math.abs(val).toLocaleString('id-ID', {minimumFractionDigits: 2})}</span>`;
                        }
                    },
            ...
            function updateSummary() {
                $.get("{{ route('approvals.stock-adjustments') }}", { summary: true }, function(data) {
                    if (data.summary) {
                        $('#pending-count').text(data.summary.pending || 0);
                        $('#approved-count').text(data.summary.approved_today || 0);
                        $('#positive-count').text(data.summary.positive_adjustments || 0);
                        $('#negative-count').text(data.summary.negative_adjustments || 0);
                    }
                });
            }
        });
