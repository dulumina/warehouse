            ...
            <div>
                <nav class="flex text-sm text-gray-500 mb-2" aria-label="Breadcrumb">
                    <ol class="flex items-center space-x-2">
                        ...
                        <li><i class="ti ti-chevron-right text-xs"></i></li>
                        <li class="text-gray-800 font-medium">Inventory Valuation</li>
                    </ol>
                </nav>
                <h2 class="text-2xl font-bold text-gray-900">Inventory Valuation</h2>
                <p class="text-sm text-gray-500 mt-1">Financial summary of current stock assets</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex items-center justify-between">
                    ...
                    <div class="bg-purple-100 rounded-full p-3">
                        <i class="ti ti-database text-purple-600 text-2xl"></i>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-500">Average Valuation Per SKU</p>
                        <p class="text-3xl font-bold text-blue-600 mt-2" id="avg-cost">Rp 0</p>
                    </div>
                    <div class="bg-blue-100 rounded-full p-3">
                        <i class="ti ti-chart-pie text-blue-600 text-2xl"></i>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200/60 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-900 flex items-center">
                    <i class="ti ti-report-money mr-2 text-blue-600"></i>
                    Valuation Breakdown
                </h3>
            ...
            <div class="p-6 border-b border-gray-200">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="w-full sm:w-64">
                        <input type="search" id="reportSearch" placeholder="Search products..."
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateSummary() {
                $.get("{{ route('reports.valuation') }}", { summary: true }, function(data) {
                    if (data.summary) {
                        $('#total-value').text('Rp ' + parseFloat(data.summary.total_value || 0).toLocaleString('id-ID', {minimumFractionDigits: 2}));
                        $('#avg-cost').text('Rp ' + parseFloat(data.summary.avg_cost || 0).toLocaleString('id-ID', {minimumFractionDigits: 2}));
                    }
                });
            }
        });
